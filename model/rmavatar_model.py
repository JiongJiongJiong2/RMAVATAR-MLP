...
class SplattingAvatarModel(GaussianBase):
    def __init__(self, config,cano_mesh,args,
        ...
        # Merge config.model into args so deformation network can access feature_type etc.
        self._deformation = deform_network(self._merge_config_args(args, config))
    ...
    ##################################################
    def _merge_config_args(self, args, config):
        """
        Merge config.model parameters into args namespace.
        This allows deformation network to access feature_type and MLP params.
        """
        ...
        # Merge config.model parameters into args
        if config is not None and hasattr(config, 'model'):
            model_config = config.model
            for key, value in model_config.items():
                # Only add if not already in args (command-line takes precedence)
