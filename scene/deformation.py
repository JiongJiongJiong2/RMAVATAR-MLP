...
from scene.mlp_feature import MLPFeatureField, MLPFeatureFieldLite
...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256,z=72,input_ch=27, input_ch_time=9, grid_pe=0, skips=[], args=None):
        ...
        # Feature field type: 'hexplane' (default) or 'mlp'
        self.feature_type = args.feature_type if hasattr(args, 'feature_type') else 'hexplane'
        
        if self.feature_type == 'mlp':
            # Use MLP-based feature field instead of HexPlane
            mlp_feat_dim = args.mlp_feat_dim if hasattr(args, 'mlp_feat_dim') else 64
            mlp_pos_pe = args.mlp_pos_pe if hasattr(args, 'mlp_pos_pe') else 6
            mlp_time_pe = args.mlp_time_pe if hasattr(args, 'mlp_time_pe') else 4
            mlp_hidden_dim = args.mlp_hidden_dim if hasattr(args, 'mlp_hidden_dim') else 256
            mlp_num_layers = args.mlp_num_layers if hasattr(args, 'mlp_num_layers') else 4
            mlp_skips = args.mlp_skips if hasattr(args, 'mlp_skips') else [2]
            mlp_lite = args.mlp_lite if hasattr(args, 'mlp_lite') else False
            
            if mlp_lite:
                self.grid = MLPFeatureFieldLite(
                    bounds=args.bounds if hasattr(args, 'bounds') else 1.6,
            ...
            print(f"[Deformation] Using MLP feature field (lite={mlp_lite})")
        else:
            # Default: use HexPlane feature field
            ...
            print("[Deformation] Using HexPlane feature field")
